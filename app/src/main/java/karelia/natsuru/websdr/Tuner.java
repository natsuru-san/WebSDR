...
public class Tuner extends Fragment {
    ...
    private BigDecimal minBorder = new BigDecimal("4.5");
    private BigDecimal maxBorder = new BigDecimal("-4.5");
    private BigDecimal freq = BigDecimal.ZERO;
    ...
    private final BigDecimal MAX_BORDER_LIMIT = new BigDecimal("6");
    private final BigDecimal MIN_BORDER_LIMIT = new BigDecimal("-6");
    private final BigDecimal MAX_BORDER_LIMIT_OUT = BigDecimal.ZERO;
    private final BigDecimal MIN_BORDER_LIMIT_OUT = BigDecimal.ZERO;
    ...
    private void resumeTuning(){
        ...
        setMode();
        minBorder = BigDecimal.valueOf(minBorderStatic);
        maxBorder = BigDecimal.valueOf(maxBorderStatic);
        ...
        freq = BigDecimal.valueOf(freqStatic);
        sendParams();
        sendAudioParams();
        setVolume();
    }
    ...
    private void setWidthChannel(boolean vector){
        switch (mode){
            ...
            case "AM":
                if(vector){
                    minBorder = minBorder.subtract(new BigDecimal("0.5"));
                    maxBorder = maxBorder.add(new BigDecimal("0.5"));
                }else{
                    minBorder = minBorder.add(new BigDecimal("0.5"));
                    maxBorder = maxBorder.subtract(new BigDecimal("0.5"));
                }
                break;
            case "LSB":
                if(vector){
                    minBorder = minBorder.subtract(new BigDecimal("0.1"));
                }else{
                    minBorder = minBorder.add(new BigDecimal("0.1"));
                }
                break;
            case "USB":
                if(vector){
                    maxBorder = maxBorder.add(new BigDecimal("0.1"));
                }else{
                    maxBorder = maxBorder.subtract(new BigDecimal("0.1"));
                }
                break;
            case "CW":
                if(vector){
                    minBorder = minBorder.subtract(new BigDecimal("0.05"));
                }else{
                    minBorder = minBorder.add(new BigDecimal("0.05"));
                }
                break;
        }
        verifyLimit();
    ...
    private void verifyLimit() {
        if (maxBorder.compareTo(MAX_BORDER_LIMIT) > 0) {
            maxBorder = MAX_BORDER_LIMIT;
        }
        if (maxBorder.compareTo(MAX_BORDER_LIMIT_OUT) < 0) {
            maxBorder = MAX_BORDER_LIMIT_OUT;
        }
        if (minBorder.compareTo(MIN_BORDER_LIMIT) < 0) {
            minBorder = MIN_BORDER_LIMIT;
        }
        if (minBorder.compareTo(MIN_BORDER_LIMIT_OUT) > 0) {
    ...
    protected void setMemory(BigDecimal freq, BigDecimal minBorder, BigDecimal maxBorder, String mode) {
        this.mode = mode;
        setMode();
        this.minBorder = minBorder;
        this.maxBorder = maxBorder;
        upBorder.setText(maxBorder.toString());
        downBorder.setText(minBorder.toString());
        firstRun = 0;
        this.freq = freq;
        tuneFreq.scrollToPosition(new BigDecimal("28991").subtract(freq).intValue());
        modulationGroup.clearCheck();
        switch (mode) {
            case "FM":
        ...
        if (main.getLastFreq().compareTo(freq) > 0){
            this.freq = this.freq.add(new BigDecimal("19"));
        }
        sendParams();
    }
